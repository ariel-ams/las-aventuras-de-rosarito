...
foreach ($shot in $shots) {
  ...
  & $EdgePath --headless=new --disable-gpu --use-gl=swiftshader --disable-software-rasterizer --no-sandbox --no-first-run --no-default-browser-check `
    --disable-background-timer-throttling --run-all-compositor-stages-before-draw --hide-scrollbars --window-size=$($shot.size) --virtual-time-budget=18000 --screenshot=$path $url | Out-Null
  Write-Output $path
}
