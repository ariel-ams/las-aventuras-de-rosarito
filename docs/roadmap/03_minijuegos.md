...
## Actualizaci�n del ciclo (2026-08-14, v18)

- Ajuste implementado:
  - Se consolid� la configuraci�n visual de partes de `DonesGameScene` y `PuzzleGameScene` en `SCENE_LAYOUTS` para evitar desviaciones de UI entre builds:
    - tarjetas de elecci�n del don
    - tarjetas de componentes con icono y label
    - panel de referencia del rompecabezas
  - Se dej� `checklist` de objetos con estilo de texto centralizado (`labelStyle` + `TEXT_STYLES.body`) para mantener consistencia de tipograf�a.
- Validaci�n en este bloque:
  - Revisiones sint�cticas y capturas con `roadmap-cycle-continue-v18`.
...
- Estado pr�ctico:
  - No se modific� mec�nica de juego.
  - Se redujeron literales visuales y se gan� estabilidad de mantenimiento para la siguiente iteraci�n de unificaci�n de UI.
